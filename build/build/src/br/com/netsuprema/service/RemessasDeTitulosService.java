...
public class RemessasDeTitulosService {
	...
	private List<File> obterArquivosProcessamento(Conta conta) throws Exception{
		try {
			...
			ScannerFilesThread.logErros.add("Carregando Arquivos - FileService");
			List<File> files;
	...
	public List<Remessa> listarRemessasProcessadasComSucessoPorData(LocalDate dataInicial, LocalDate dataFinal) {
		...
		try {
			session = factory.openSession();
			List<Remessa> remessas = new RemessaRepository(session).listarRemessasProcessadasComSucessoPorData(dataInicial, dataFinal);
			return remessas;
		} finally {
			if ((session != null) && (session.isOpen())) {
				session.close();
	...
	public List<Remessa> listarRemessasProcessadasComErroPorData(LocalDate dataInicial, LocalDate dataFinal) {
		...
		try {
			session = factory.openSession();
			List<Remessa> remessas = new RemessaRepository(session).listarRemessasProcessadasComErroPorData(dataInicial, dataFinal);
			return remessas;
		} finally {
			if ((session != null) && (session.isOpen())) {
				session.close();
	...
	public List<Remessa> listarRemessasEnviadasPorData(LocalDate dataInicial, LocalDate dataFinal) {
		...
		try {
			session = factory.openSession();
			List<Remessa> remessas = new RemessaRepository(session).listarRemessasEnviadasPorData(dataInicial, dataFinal);
			return remessas;
		} finally {
			if ((session != null) && (session.isOpen())) {
				session.close();
	...
	public List<Remessa> listarRemessasComAguardoProcessamentoServidorPorData(LocalDate dataInicial, LocalDate dataFinal) {
		Session session = null;
		try {
			session = factory.openSession();
			List<Remessa> remessas = new RemessaRepository(session).listarRemessasAguardandoProcessamentoServidorPorData(dataInicial, dataFinal);
			return remessas;
		} finally {
			if ((session != null) && (session.isOpen())) {
				session.close();
	...
	public List<Remessa> listarTodasRemessasPorParametro(LocalDate dataInicial, LocalDate dataFinal, Integer codigoCedente, Integer codigoEnvio, Integer codigoConta, StatusProcessamento status){
		Session session = null;
		try {
			session = factory.openSession();
			List<Remessa> remessas = new RemessaRepository(session).listarTodaRemessasPorParametros(dataInicial, dataFinal, codigoCedente, codigoEnvio, codigoConta, status);
			return remessas;
		} finally {
			if ((session != null) && (session.isOpen())) {
				session.close();
